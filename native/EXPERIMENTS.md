...
| **PRF / query expansion (union, production anchor selection)** | P@10 0.820→0.778 (hurts), P@sup 0.430→0.433 (null). Redundancy: anchor↔anchor cos 0.938 vs anchor↔tail 0.857 | **OWNER CALLED IT before it ran: "pointing hands at itself."** Anchors are near-copies of the query's view; 37% of the tail is nominally reachable but anchors' false positives outrank it. Centroid variants would be worse. With this, EVERY label-free read-side mechanism is measured dead: diffusion, verification re-rank, PRF |
| Sakoe-Chiba band (as first implemented) | P@10 0.765→0.203 at every M | banded around the PADDED width: short references' real cells forbidden. Fixed per-reference 2026-08-17; band=0.5 now bit-exact vs full |
