...
def main():
    ...
    ap.add_argument("--primary", default="a", choices=["a", "b"],
                    help="which of pred_change / obs_change enters f as a "
                         "VECTOR. The other enters only through g's two "
                         "scalars, so a-b is never formable either way.")
    ...
    torch.save(dict(state=model.state_dict(), dim=a.dim, use_sig=not a.no_sig,
                    ...
                    phase=a.phase, arc=arc, primary=a.primary,
